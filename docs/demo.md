...
- `strujam8-demo.gif`: short interaction loop generated by `npm run demo:capture`.
...
The capture script uses a clean browser profile and does not click Play, so the
GIF is safe to generate from the public Pages URL as well as a local server.

```bash
DEMO_URL=https://toyo1621.github.io/StruJam8/ npm run demo:capture
```
...
- Capture desktop, tablet, and the demo GIF after `npm run check` and `npm run test:e2e` pass.
